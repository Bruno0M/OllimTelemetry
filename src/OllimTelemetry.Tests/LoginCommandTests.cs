...
public sealed class LoginCommandTests : IDisposable
{
    ...
    public LoginCommandTests() => Directory.CreateDirectory(_tempDir);
    ...
    public async Task ExecuteAsync_DevGuard_ReturnsOneWithoutHttpCall()
    {
        ...
        var result = await LoginCommand.ExecuteAsync(http, manager, config, "dev", TimeSpan.FromMinutes(5), NoBrowser);
    ...
    public async Task ExecuteAsync_HappyPath_SavesConfigAndReturnsZero()
    {
        ...
        var result = await LoginCommand.ExecuteAsync(http, manager, config, null, TimeSpan.FromMinutes(5), NoBrowser);
    ...
    public async Task ExecuteAsync_Timeout_ReturnsOne()
    {
        ...
        var result = await LoginCommand.ExecuteAsync(
