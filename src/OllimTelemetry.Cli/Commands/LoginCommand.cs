        ...
        Action<string>? browserOpener = null)
    {
        ...
        if (env == "dev" && config.BackendUrl.Contains("ollim.dev", StringComparison.OrdinalIgnoreCase))
        {
            await Console.Error.WriteLineAsync(
                "[ollim] error: cannot login to the production API from a dev environment. " +
        ...
        {
            await Console.Error.WriteLineAsync($"[ollim] error starting login: {ex.Message}");
            return 1;
        }
        ...
        await AnsiConsole.Status()
            ...
            .StartAsync("Waiting for GitHub authorization…", async _ =>
            {
                while (DateTime.UtcNow < deadline)
                {
                    ...
                    {
                        errMsg = "session expired. Run `ollim login` again.";
                        return;
                    }
                ...
                errMsg = "timed out waiting for authorization. Run `ollim login` again.";
            });
