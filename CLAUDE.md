...
**Ollim Telemetry** is a NativeAOT CLI tool (`ollim`) that hooks into Claude Code's Stop event to extract token usage from local JSONL logs (`~/.claude/projects/**/*.jsonl`) and (with explicit opt-in) submits anonymized data to `api.ollim.dev`. There is no background daemon — everything runs via the Stop hook. Config lives in `~/.config/ollim/` and the SQLite queue in `~/.local/share/ollim/` (XDG Base Directory spec).
